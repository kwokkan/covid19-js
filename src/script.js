...
(async function () {
    ...
    const allData = await d3.csv(app.getAttribute("data-data-url"));
    const filteredData = allData.filter(x => x.geo_type === "country/region" && x.transportation_type === "driving");
    ...
    const countryData = filteredData.map(x => ({
        name: x.region,
